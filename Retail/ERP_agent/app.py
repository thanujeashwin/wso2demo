...
app = FastAPI(title="Morrisons Oracle ERP Agent", version="1.0.0")
...
@app.get("/health")
def health():
    return {"status": "ok", "agent": "morrisons-oracle-erp-agent"}
...
def chat(request: ChatRequest) -> ChatResponse:
    ...
    thread = f"{user_id}:{request.session_id}" if user_id else f"anonymous:{request.session_id}"
    try:
