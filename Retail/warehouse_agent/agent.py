"""agent.py — Warehouse Agent with LLM-driven ReAct loop.
...
_TOOL_ROUTES = [
    (["dispatch", "status", "update", "shipped", "delivered"], "update_dispatch_status",
     {"task_id": "TASK-UNKNOWN", "status": "dispatched"}),
    (["assign", "picker", "pick"],  "assign_picker", {"task_id": "TASK-UNKNOWN"}),
    (["fulfil", "task", "order", "pack", "create"], "create_fulfilment_task",
     {"order_id": "ORD-UNKNOWN", "items": [{"product_id": "PROD-001", "quantity": 1}]}),
]
_DEFAULT_TOOL = ("create_fulfilment_task", {"order_id": "ORD-UNKNOWN", "items": []})
...
class GatewayLLM:
    ...
    _SYSTEM_PROMPT = (
        "You are a warehouse fulfilment agent for a retail supermarket. "
        "Your job is to create pick-and-pack fulfilment tasks for orders, assign pickers, "
        "and update dispatch statuses. "
        "Always use the available tools. Be precise and concise."
    )

    _TOOL_DESCRIPTIONS = (
        "create_fulfilment_task(order_id: string, items: [{product_id, quantity}]) — create a pick-and-pack task for an order\n"
        "assign_picker(task_id: string) — assign an available picker to a fulfilment task\n"
        "update_dispatch_status(task_id: string, status: string) — update the dispatch status of a task"
    )
...
class DemoLLM:
    ...
    def _extract_entities(self, text: str, tool_name: str, args: dict) -> dict:
        ...
        task_m = _TASK_RE.search(text.upper())
        ...
        if task_m and "task_id" in args:
            args["task_id"] = task_m.group(0)
        if tool_name == "create_fulfilment_task":
...
def run(message: str, session_id: str, context: dict | None = None) -> str:
    ...
    with start_span("warehouse_agent.chat", attributes={
